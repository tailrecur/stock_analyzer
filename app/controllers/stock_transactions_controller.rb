...
class StockTransactionsController < InheritedResources::Base
  ...
  def create
    portfolio_stock = PortfolioStock.find_or_create_by_portfolio_id_and_company_id(@portfolio.id, params.delete(:company_id))
  ...
  def new
    @stock_transaction = StockTransaction.new(:transaction_date => @portfolio.start_date)
  end
